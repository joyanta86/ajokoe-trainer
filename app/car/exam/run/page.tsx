...
import { formatDuration, toQuizQuestion } from "@/lib/exam-logic";
...
export default function ExamRunPage() {
  ...
  const recordManyAttempts = useProgressStore((state) => state.recordManyAttempts);
  ...
  const examQuestions = useMemo(
    () => (active ? getQuestionsByIds(active.questionIds) : []),
    [active],
  );
  ...
  const secondsLeft = hydrated ? remainingSeconds(active) : 0;
  ...
  useEffect(() => {
    if (hydrated && !active && !submittingRef.current) router.replace("/car/exam");
      ...
      {showConfirm ? (
        ...
        >
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <h2 id="submit-title" className="text-lg font-semibold text-ink-900">
              Submit your exam?
            </h2>
